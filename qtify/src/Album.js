...
const Albums = ({ title, apiUrl, data = [] }) => {
  return (
    <section className={styles.albums}>
      ...
      <div className={styles.albumList}>
        {data.map((item, index) => (
          <Card key={item?.id || index} className={styles.albumCard}>
            <img src={item?.image} alt={item?.title} className={styles.albumImage} />
            <CardContent>
              <Typography variant="h6">{item.title}</Typography>
              <Chip
                label={title === "Songs" ? `${item.likes} Likes` : `${item.follows} Follows`}
                color="primary"
              />
            </CardContent>
          </Card>
